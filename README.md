...
这是一个可本地启动的实时语音转写演示项目，包含 SSO 登录、Web 多人语聊房、实时字幕、翻译字幕、移动端扫码字幕查看和 VTT 字幕下载能力。
...
SSO_SESSION_SECRET="替换为本地 session 签名密钥，建议至少 32 位随机字符串"
...
| `SSO_SESSION_SECRET` | 本应用本地 session JWT 签名密钥。建议使用随机长字符串。 |
